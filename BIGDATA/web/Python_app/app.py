...
def predict_func(x1,x2,x3,x4,x5,x6,x7,x8):
    input_data = pd.DataFrame({'성별': [x1], '거주지역': [x2], '연령': [x3], '결제등록카드': [x4], 
                               '유입경로': [x5], '자녀여부': [x6], '구매일로부터기간': [x7], '총결제금액': [x8]})
    result = model.predict(input_data)
    if result == 1:
        result = '1번 이후 주문하지 않을 것입니다.'
    else :
        result = '이후 계속 주문할 가능성이 높습니다.'
    return result

#########################################################################################
input_layout = [
    #['성별', '거주지역', '연령', '결제등록카드', '유입경로', '자녀여부', '구매일로부터기간', '총결제금액']
    ## drop drop input drop drop drop input input
    html.Div([
        html.Label('성별'),
        dcc.Dropdown(
            id='my_sex',
...
app.layout = dbc.Container(
    # Layer 틀을 구성 
    [
        html.H1(children='For U Kids',style={'background-color': '#6495ed',
                                             'text-align': 'center',
                                             'line-height': '150%',
                                             'color' : 'white'}),
        ...
        html.Hr(), # 밑줄 
        dbc.Card([
            html.P('해당고객은 이 후 물품을', className='card_text'),
